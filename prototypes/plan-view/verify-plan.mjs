...
check('sidebar source links keep readable source labels', /appendDetailSourceLinks\(els\.detailSources, sourceIds\)/.test(app) && /getSourceBaseLabel/.test(app) && /\$\{baseLabel\} \$\{seen\}/.test(app));
check('room overview hotspots share room details', hotspotMap.get('kv62-comparison')?.detailRoomId === 'corridor' && hotspotMap.get('storage-supplies')?.detailRoomId === 'annex' && !hotspotMap.has('first-glimpse') && /hotspot\.detailRoomId/.test(app));
